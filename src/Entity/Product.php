...
class Product
{
    ...
    /**
     * @ORM\Column(type="string", length=255)
     * @Assert\NotBlank(message="Vous devez choisir une illustration.")
     * @Assert\Regex (
     *     pattern="#\.(jpg|jpeg|png|gif)$#",
     *     match=true,
     *     message="L'image ne possède pas une extension valide (jpg, jpeg, png, ou gif)."
     * )
     */
    ...
    public function getAvgRatings()
    {
        ...
        return count($this->getComments()) == 0 ? 0
            : round($sum / count($this->getComments()));
    }
    ...
    public function isProductFromUser($user)
    {
        foreach ($user->getOrders() as $order) {
            foreach ($order->getOrderDetails() as $detail) {
                if (
                    $detail->getProduct()->getId() == $this->id
                    && $order->getStatus()
                )
